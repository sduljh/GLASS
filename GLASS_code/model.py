...
def save_preprocessed_data(graph, graph_file):
    dgl.save_graphs(graph_file, [graph])
    print(f"Preprocessed data saved: {graph_file}")
...
def data_train(train_junction_gtf_file = None, train_bam_files = None, val_bam_file = None, val_junction_gtf_file = None, train_device ='cpu'):
    ...
    if val_bam_file is None or val_junction_gtf_file is None:
        ...
        junction_df = process_junction(val_junction_gtf_file)


        # Call function
        feature_df, intron_df = train_preprocess(val_bam_file, junction_df)
        val_graph = cons_graph(feature_df, intron_df, device = train_device)
        val_graph_file = f"{os.path.splitext(os.path.basename(val_bam_file))[0]}.dgl"
        save_preprocessed_data(val_graph, val_graph_file)

        val_graph = val_graph.to(train_device)
        val_labels = val_graph.nodes['read'].data['label'].to(train_device)
    ...
    if train_junction_gtf_file is None or train_bam_files is None:
        ...
        train_junction_df = process_junction(train_junction_gtf_file)
        models = []
        model_f1_scores = []
        for alignment_bam_path in train_bam_files:
            ...
            # Call function
            feature_df, intron_df = train_preprocess(alignment_bam_path, train_junction_df)
            train_graph = cons_graph(feature_df, intron_df, device = train_device)
            train_graph_file = f"{os.path.splitext(os.path.basename(alignment_bam_path))[0]}.dgl"
            save_preprocessed_data(train_graph, train_graph_file)
            train_graph = train_graph.to(train_device)
...
def test_model(models, weights, graph, test_device = 'cpu'):
    ...
    with torch.no_grad():
        ...
        # labels = graph.nodes['read'].data['label']
...
def data_test(alignment_bam_path, models, weights, test_device):
    feature_df, intron_df = test_preprocess(alignment_bam_path)
    graph = cons_graph(feature_df, intron_df, device='cpu')
