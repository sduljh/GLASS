...
def extract_read_features(graph, feature_info, selected_columns, device, mode='train'):
...
def train_consgraph(feature_df, intron_df, device='cpu'):
    ...
    selected_columns = [
        'read_id2_intron_number', 'mapped-length_percent', 'SOFT_CLIP_Left_percent',
        'SOFT_CLIP_right_percent', 'CIGAR_score', 'unique_intron_number_average',
        'unique_intron_number_1_average', 'chaining_score', 'start_align_average',
        'end_align_average'
    ]
    graph = extract_read_features(graph, read_features, selected_columns, device, mode='train')

    # Add intron_nodes to the graph
    graph = add_intron_nodes(graph, intron_info, read_features)

    logger.info("Training graph creation complete.")
    return graph


def test_consgraph(feature_df, intron_df, device='cpu'):
    ...
    graph = extract_read_features(graph, read_features, selected_columns, device, mode='test')

    # Add intron_nodes to the graph
    graph = add_intron_nodes(graph, intron_info, read_features)

    logger.info("Testing graph creation complete.")
    return graph
