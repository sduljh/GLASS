...
*/	
	CReadAln(GStr rname, char _strand=0, short int _nh=0, //yuting GStr rname
			int rstart=0, int rend=0, TAlnInfo* tif=NULL): GSeg(rstart, rend), 
					name(rname), //yuting open
					strand(_strand),nh(_nh), len(0), read_count(0), unitig(false),pair_count(),pair_idx(),
					segs(), juncs(false), tinfo(tif) { }
