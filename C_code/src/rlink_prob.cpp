...
//		fprintf(stderr,"check junction:%d-%d:%d leftsupport=%f rightsupport=%f nm=%f nreads=%f\n",junction[i]->start,junction[i]->end,junction[i]->strand,junction[i]->leftsupport,junction[i]->rightsupport,junction[i]->nm,junction[i]->nreads); //yuting open 
	...
	for(int i=0;i<g_junction.size();i++) cout<<"junction_yu: "<<bdata->refseq.text()<<" "<<g_junction[i].first<<" "<<g_junction[i].second<<" "<<g_junction_coverage[i]<<" "<<g_junction_strand[i]<<endl;
...
cout<<"here"<<" "<<i<<endl;
		if(rd.juncs.Count() == 0){
		    ...
		    cout<<"  junc between exons:"<<"*"<<" "<<"*"<<": "<<rd.segs[i].start<<"-"<<rd.segs[i].end<<"->"<<rd.segs[i+1].start<<"-"<<rd.segs[i+1].end
			    <<" strand: "<<strand_yu<<'\n';
		}
		cout<<"here2"<<endl;
		...
		//if(keep) { // if it's a good read that needs to be kept
			...
			//color=add_read_to_group(n,readlist,color,group,currgroup,startgroup,readgroup,equalcolor,merge); //yuting is useful for me
...
if(assembly_flag) {
	...
	if(0 && (bundledist || (guides.Count() && !longreads)) ) { //yuting
